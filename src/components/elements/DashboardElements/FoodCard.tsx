...
const FoodCard: React.FC<CardProps> = ({ id,img, nama, description, statusPickUp }) => {
  ...
  const handleStatusChange = async () => {
    ...
    const token = Cookies.get('token');
      if (token) {
        try {
          const response = await axios.patch(`http://localhost:3001/api/food-donation/picked-up/${id}`, {
            headers: {
